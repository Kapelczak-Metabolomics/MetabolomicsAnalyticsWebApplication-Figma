...
router.get("/stats", async (_req: Request, res: Response) => {
  const [users, projects, running, alerts] = await Promise.all([
...
router.get("/logs", async (_req: Request, res: Response) => {
  const result = await query<{
    ...
    `SELECT id, level, user_email, action, details, ip, created_at FROM system_logs ORDER BY created_at DESC LIMIT 50`
  );
